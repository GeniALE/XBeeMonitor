...
/***************************************************************************
 * @prototype       uint8_t do_aci_setup(aci_state_t *aci_stat)
 * @description     Setup the nRF8001 device
 ...
 *                  otherwise
 * @param           aci_stat: ACI state structure
 * @return          An integer indicating the reason the function terminated
 ***************************************************************************/
uint8_t do_aci_setup(aci_state_t *aci_stat)
{
    ...
    // If it is non-empty we return. The user should then process the messages before calling do_aci_setup() again
    if (!hal_aci_tl_tx_q_empty())
    {
    ...
    // so that the user can handle them. At this point we don't care what the event is, as any event is an error.
    ...
    {   // This counter is used to ensure that this function does not loop forever. When the device
        ...
        if (lib_aci_event_peek(aci_data))
        {
            ...
            // or ACI_STATUS_TRANSACTION_COMPLETE. We don't need the event itself, so we simply remove it from the queue
...
/***************************************************************************
 * @prototype       static bool aci_setup_fill(aci_state_t *aci_stat, uint8_t *num_cmd_offset)
 * @description     Utility function to fill the the ACI command queue
 * @param           aci_stat: Pointer to the ACI state
 *                  num_cmd_offset(in/out): Offset in the Setup message array to
 *                  start from offset is updated to the new index after the queue
 *                  is filled or the last message us placed in the queue
 * @return          True if at least one message was transferred
 ***************************************************************************/
